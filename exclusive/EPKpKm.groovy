...
class EPKpKm {
  static def getEPKpKm(HipoDataEvent event) {
    ...
    def findKp = { pbank -> (0..<pbank.rows()).findAll{pbank.getInt('pid',it)==321}
      .max{ind -> (new Vector3(*['px', 'py', 'pz'].collect{pbank.getFloat(it,ind)})).mag2()}
    }
    def findKm = { pbank -> (0..<pbank.rows()).findAll{pbank.getInt('pid',it)==-321}
      .max{ind -> (new Vector3(*['px', 'py', 'pz'].collect{pbank.getFloat(it,ind)})).mag2()}
    }

    def inds = []
    for(def findPart in [findElectron, findProton, findKp, findKm]) {
    ...
    def parts = [11,2212,321,-321].withIndex()
